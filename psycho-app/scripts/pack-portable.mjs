...
const blanksSrc = path.join(srcDir, 'original-blanks')
if (fs.existsSync(blanksSrc)) {
  copyDir(blanksSrc, path.join(docsDir, 'original-blanks'))
  console.log('OK docs/original-blanks синхронизирован')
}
...
// Prefer system zip (Linux/macOS); fall back to PowerShell on Windows
try {
  execFileSync('zip', ['-r', '-q', zipPath, path.basename(srcDir)], {
    stdio: 'inherit',
    cwd: root,
  })
  const kb = Math.round(fs.statSync(zipPath).size / 1024)
  console.log(`OK ${zipPath} (${kb} KB)`)
} catch {
  const ps = `
...
`
  try {
    ...
    console.warn('ZIP не создан (нет zip/PowerShell) — HTML и docs уже готовы.')
  }
}
